...
######
######
###### COMMENT: THIS CODE FAILS DUE TO THE RELAXATION. IMPLEMENT CARRIE'S EMAIL INFO TO FIX
######
######
...
def run_optimizer_relax_boundary(ode, IG, BoundRange, BoundaryFirst, BoundaryLast, optType, E_or_T, Umax, Umin, numKnots, numThreads, MeshTol, EControl):
    phase = ode.phase(optType, IG, numKnots)
    #Fix first state and time
    phase.addBoundaryValue("First", BoundRange, BoundaryFirst) #Q: Does this have constraints on the dimensionality? A: 1d array if this doesnt work
    #Fix last state and time
    phase.addBoundaryValue("Last", BoundRange, BoundaryLast)
    # Bound control forces
    phase.addLUNormBound("Path",[7,8,9],Umin,Umax) #Q: how to write this line? A: LUNorm - don't let it go to 0
    #Produce a mass-optimal result by integrating over a norm() applied to the thrust vector
    if E_or_T == 0:
        ...
        phase.addIntegralObjective((Args(3).norm()-Umax*tf).abs(),[7,8,9])
...
def Full_Plot(Traj,IG,ref_state):
    ...
    ax0.plot(TU/86400 * IG_array[6], DU/TU**2 * IG_array[7], color='r')
    ax1.plot(TU/86400 * Traj_array[6], DU/TU**2 * Traj_array[8])  # plots u_y vs time
    ax1.plot(TU/86400 * IG_array[6], DU/TU**2 * IG_array[8], color='r')
    ax2.plot(TU/86400 * Traj_array[6], DU/TU**2 * Traj_array[9])  # plots u_z vs time
    ax2.plot(TU/86400 * IG_array[6], DU/TU**2 * IG_array[9], color='r')
    ax3.plot(TU/86400 * Traj_array[6], Thrust_Mass_Mag)     # plots u mag vs time
    ax3.plot(TU/86400 * IG_array[6], Thrust_Energy_Mag, color='r')     # plots u mag vs time
...
if __name__ == "__main__":
    ...
    EControl = 1.0e-10
    BoundRange = [0,1,6]
    BoundaryFirst = [IG_state[0,0]] + [IG_state[1,0]] + [0]
    BoundaryLast = [IG_state[0,0]] + [IG_state[1,0]] + [tf]

    phase = run_optimizer_relax_boundary(ode, IG, BoundRange, BoundaryFirst, BoundaryLast, optType, E_or_T, Umax, Umin, numKnots, numThreads, MeshTol, EControl)
    Traj = phase.returnTraj()
    ...
    phase2 = run_optimizer_relax_boundary(ode, IG, BoundRange, BoundaryFirst, BoundaryLast, optType, E_or_T, Umax, Umin, numKnots, numThreads, MeshTol, EControl)
    Traj = phase2.returnTraj()
    ...
    ##############################################
    ##############################################
    # ADDING EXTRA BELOW TO FIND MORE
    ...
    IG_front = IG_state[:6,0] + np.array([-0.02, 0., -IG_state[2,0]*0.2, 0., 0., 0.])
    
    phase = ode.phase("LGL3", IG, 64)
    #Fix first state and time
    phase.addBoundaryValue("First", range(0,7), list(IG_front) + [0]) #Q: Does this have constraints on the dimensionality? A: 1d array if this doesnt work
    #Fix last state and time
    phase.addBoundaryValue("Last", range(0,7), list(IG_front) + [tf])
    # Bound control forces
    phase.addLUNormBound("Path",[7,8,9],1e-8,Umax) #Q: how to write this line? A: LUNorm - don't let it go to 0
    #Produce a mass-optimal result by integrating over a norm() applied to the thrust vector
    phase.addIntegralObjective(Args(3).squared_norm(),[7,8,9]) #Q: I belive this line works. What do you think? A: Yes
    ...
    phase.setThreads(8,8) #Q: what does this do? A:Parallelization
    phase.optimizer.set_PrintLevel(1)
    phase.optimize()
    ...
    phase2 = ode.phase("LGL3", IG, 100)
    #Fix first state and time
    phase2.addBoundaryValue("First", range(0,7), list(IG_front) + [0]) #Q: Does this have constraints on the dimensionality? A: 1d array if this doesnt work
    #Fix last state and time
    phase2.addBoundaryValue("Last", range(0,7), list(IG_front) + [tf])
    # Bound control forces
    phase2.addLUNormBound("Path",[7,8,9],1e-8,Umax) #Q: how to write this line? A: LUNorm - don't let it go to 0
    #Produce a mass-optimal result by integrating over a norm() applied to the thrust vector
    phase2.addIntegralObjective(Args(3).norm(),[7,8,9]) #Q: I belive this line works. What do you think? A: Yes

    
    # here, we are adding adaptive meshing
    phase2.setAdaptiveMesh(True)  #Enable Adaptive mesh for all following solve/optimize calls
    phase2.setMeshErrorEstimator('integrator')     #default
    ## Set Error tolerance on mesh (epsilon)
    phase2.setMeshTol(1.0e-10)  #default = 1.0e-6 
    ## Make sure to set optimizer EContol to be the same as or smaller than MeshTol
    phase2.optimizer.set_EContol(1.0e-12)
    ## Set Max number of mesh iterations:
    phase2.setMaxMeshIters(10)  #default = 10
    
    phase2.setThreads(8,8) #Q: what does this do? A:Parallelization
    phase2.optimizer.set_PrintLevel(1)
    phase2.optimize()
    ...
    IG_front = IG_state[:6,0] #+ np.array([-0.02, 0., -IG_state[2,0]*0.2 + 0.05, 0., 0., 0.])
    ...
    # Going to introduce an InterpTable to integrate the trajectory under the real dynamics with the thrust
    # Construct from arbitrary time series data. Elements consist of data followed by time. No ode needed, but less accurate interpolation
    #JustUts = [list(T[7:]) +[T[6]] for T in Traj]
    Tab = phase2.returnTrajTable()

    integTab = ode.integrator(0.00001,Tab)
    integTab.setAbsTol(1.0e-14)
    integTab.setRelTol(1.0e-14)
